...
public class Encoding {
	...
	private void calculateSegmentSizes() {
		...
		segmentSizes_2s = new int[refCount];
		segmentSizes_4s = new int[(int)Math.ceil(refCount / 2.0)];
		segmentSizes_8s = new int[(int)Math.ceil(refCount / 4.0)];
		segmentSizes_16s = new int[(int)Math.ceil(refCount / 8.0)];
		...
		for (int i = 0; i < refCount; i++) {
			...
			segmentSizes_2s[i] += currentSize;
			segmentSizes_4s[i/2] += currentSize;
			segmentSizes_8s[i/4] += currentSize;
			segmentSizes_16s[i/8] += currentSize;
